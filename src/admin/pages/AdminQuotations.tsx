    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">{filtered.length} báo giá</p>
        </div>
      </div>
            ...
            <tbody>
              {filtered.map(q => (
                <tr key={q.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 text-amber-600/80 text-xs font-mono">{q.quotation_code}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-slate-800 text-sm">{q.products?.name ?? '—'}</p>
                        <p className="text-slate-500 text-xs font-mono">{q.products?.code}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right text-slate-500 text-sm">{formatCurrency(q.listed_price)}</td>
                  <td className="px-6 py-4 text-center text-red-400 text-sm font-medium">{q.discount_percent}%</td>
                  <td className="px-6 py-4 text-right text-amber-600 font-semibold text-sm">{formatCurrency(q.final_price)}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-1">
                      {q.status === 'pending' && (
                        <>
                          <button onClick={() => updateStatus(q.id, 'confirmed')} className="text-xs px-2 py-1 rounded-lg bg-emerald-100 text-emerald-600 hover:bg-emerald-500/20 transition-colors">Xác nhận</button>
