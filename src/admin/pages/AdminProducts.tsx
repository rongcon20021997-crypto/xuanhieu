    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{filtered.length} sản phẩm</p>
        </div>
        {canEdit && (
          <button
            ...
            <tbody>
              {filtered.map(product => (
                <tr key={product.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-400 text-sm">{product.categories?.name || '—'}</td>
                  <td className="px-6 py-4 text-right text-amber-400 font-semibold text-sm">{formatCurrency(product.listed_price)}</td>
                  <td className="px-6 py-4 text-center">
                    <span className={`inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full font-medium ${product.stock_status === 'in_stock' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                  ...
                  <td className="px-6 py-4 text-center">
                    {canEdit ? (
                      <button onClick={() => toggleVisible(product)} className={`p-1.5 rounded-lg transition-all ${product.is_visible_ipad ? 'text-emerald-400 hover:bg-emerald-500/10' : 'text-slate-600 hover:bg-slate-700'}`}>
                        {product.is_visible_ipad ? <Eye size={16} /> : <EyeOff size={16} />}
                      </button>
                    ) : (
                      product.is_visible_ipad ? <Eye size={16} className="text-emerald-400 mx-auto" /> : <EyeOff size={16} className="text-slate-600 mx-auto" />
                    )}
                  </td>
                  <td className="px-6 py-4">
                    {canEdit && (
                      <div className="flex items-center justify-end gap-2">
                        <button onClick={() => setEditProduct(product)} className="p-2 text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg transition-all">
                          <Edit2 size={16} />
                        </button>
                      </div>
                    )}
