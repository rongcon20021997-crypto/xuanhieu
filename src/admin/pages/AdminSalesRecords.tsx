  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Bán hàng</h1>
          <p className="text-slate-400 text-sm mt-1">{filtered.length} giao dịch · Doanh thu: <span className="text-amber-400 font-semibold">{formatCurrency(totalRevenue)}</span></p>
        </div>
        <button
                ...
                <tr key={r.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-white text-sm">{r.products?.name ?? '—'}</p>
                        <p className="text-slate-500 text-xs font-mono">{r.products?.code}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right text-slate-400 text-sm">{formatCurrency(r.listed_price)}</td>
                  <td className="px-6 py-4 text-center text-red-400 text-sm">{r.discount_percent}%</td>
                  <td className="px-6 py-4 text-right text-amber-400 font-semibold text-sm">{formatCurrency(r.final_price)}</td>
                  <td className="px-6 py-4 text-slate-400 text-sm">{r.sale_user_name}</td>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-semibold text-lg">Ghi nhận bán hàng</h3>
              <button onClick={() => setShowForm(false)}><X size={20} className="text-slate-500" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-slate-400 text-sm block mb-1.5">Sản phẩm *</label>
                <select value={form.product_id} onChange={e => { const p = products.find(p => p.id === e.target.value); setForm(f => ({ ...f, product_id: e.target.value, final_price: p?.listed_price?.toString() ?? '' })); }} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500">
              ...
              {selectedProduct && (
                <div className="bg-slate-800/50 rounded-xl p-3 text-sm text-slate-400">
                  Giá niêm yết: <span className="text-amber-400 font-semibold">{formatCurrency(selectedProduct.listed_price)}</span>
                </div>
              )}
              <div>
                <label className="text-slate-400 text-sm block mb-1.5">% Giảm giá</label>
                <input type="number" value={form.discount_percent} onChange={e => { const disc = parseFloat(e.target.value) || 0; const price = selectedProduct ? selectedProduct.listed_price * (1 - disc / 100) : 0; setForm(f => ({ ...f, discount_percent: e.target.value, final_price: Math.round(price).toString() })); }} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500" min="0" max="100" />
              </div>
              <div>
                <label className="text-slate-400 text-sm block mb-1.5">Giá bán thực tế (VNĐ)</label>
                <input type="number" value={form.final_price} onChange={e => setForm(f => ({ ...f, final_price: e.target.value }))} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500" />
              </div>
              <div>
                <label className="text-slate-400 text-sm block mb-1.5">Ghi chú</label>
                <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} rows={2} className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white text-sm focus:outline-none focus:border-amber-500 resize-none" />
              </div>
            </div>
